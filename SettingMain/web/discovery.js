...
function backendCorePath(path) { return ['/api/center','/api/center-box','/api/vla/tour'].includes(path) || path.startsWith('/api/discovery/') ? `${path}${path.includes('?') ? '&' : '?'}useBackendCore=1` : path; }
